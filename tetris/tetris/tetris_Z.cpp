...
//  @ @
//    # @
int tetris_Z_0::update_map_top_line(
                    tetris_handler & handler)
{
    ...
    if(status < 0)
    {
        cerr << "tetris_Z hasn't the status: "
            << status
            << endl;
    ...
    handler.set_tetris(new tetris_Z_1);
    return handler.update_map_top_line();
}

int tetris_Z_0::hit_test_transfer(
                tetris_handler & handler)
{
    int status = handler.get_status();
    if(0 == status)
...
int tetris_Z_0::set_next_status(
            tetris_handler & handler)
{
    int status = handler.get_status();
    if(0 == status)
...
//  @ #
//  @ 
int tetris_Z_1::update_map_top_line(
                    tetris_handler & handler)
{
    int status = handler.get_status();
    if(1 == status)
    ...
    handler.set_tetris(new tetris_Z_0);
    return handler.update_map_top_line();
}

int tetris_Z_1::hit_test_transfer(
                tetris_handler & handler)
{
    int status = handler.get_status();
    if(1 == status)
...
int tetris_Z_1::set_next_status(
            tetris_handler & handler)
{
    int status = handler.get_status();
    if(1 == status)
